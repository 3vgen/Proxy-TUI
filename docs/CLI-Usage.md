...
(ставится в venv `.venv/`), минималистичный: компактная статусная строка сверху
(`VPN ВКЛ/ВЫКЛ · узел · egress`) с **кнопкой вкл/выкл VPN** справа, таблица узлов
и ряд кнопок внизу (`Auto`, `Best`, `Обновить`, `Выход`). Кнопки кликабельны
мышью; всё продублировано клавишами. UI не блокируется во время операций
(асинхронно + спиннер).

Кнопки:
| Кнопка | Действие |
|---|---|
| `Выключить/Включить VPN` | вкл/выкл глобального VPN (systemctl start/stop sing-box); подпись и цвет меняются по состоянию |
| `Auto` | подключить `auto` (urltest) |
| `Best` | подключить `best` (лучший зарубежный) |
| `Обновить` | обновить подписку |
| `Выход` | выйти из TUI |
